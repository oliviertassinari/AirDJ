...
public class KinectListener implements ListenerInterface {
	...
	public void ListenToKinect(KinectEvent event) {
		ModeleKinect modeleKinect = event.getSource().getModele()
				.getModeleKinect();
		...
		if (event.getMessage() == "volume") {
			...
			ModeleCrossfinder crossfinder = event.getSource().getModele()
					.getModeleCrossfinder();
			if (event.getCote() == "left") {
				crossfinder.setVolumeP1(crossfinder.getVolumeP1()
						+ event.getValeur());
				modeleKinect.setMessageGauche("volume");
				modeleKinect.setMessageDroite(null);
			} else if (event.getCote() == "right") {
				crossfinder.setVolumeP2(crossfinder.getVolumeP2()
						+ event.getValeur());
		...
		if (event.getMessage() == "crossfinder") {
			ModeleCrossfinder crossfinder = event.getSource().getModele()
					.getModeleCrossfinder();
			crossfinder.setCrossfinder(crossfinder.getCrossfinder()
					+ event.getValeur());
